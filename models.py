...
def db_init():
    ...
    user1 = User("abahuguna", bcrypt.generate_password_hash("mypwd"), "Arch",\
                  "Bahuguna", "9194138961", "abahuguna@hotmail.com")
    db.session.add(user1)
    db.session.commit()
    user2 = User("User2", bcrypt.generate_password_hash("pwd2"), "user2",\
                  "Tibby", "9194138865", "user2@hotmail.com")
    db.session.add(user2)
    db.session.commit()

    event1 = Event("Event1", datetime.now(), \
                   "May not be able to do a good job", "Blank",\
                   8, 0, FEARAPP_STATUS_CREATED, 0, user1.userid)
    db.session.add(event1)
    event2 = Event("Event2", datetime.now(), \
                   "Not feeling so confident about the ppt", "Blank",\
                   8, 0, FEARAPP_STATUS_CREATED, 0, user2.userid)
    db.session.add(event2)
    event3 = Event("Event3", datetime.now(), \
                   "Not feeling so confident about the ppt", "Blank",\
                   7, 2, FEARAPP_STATUS_UPDATED, 0, user2.userid)
    db.session.add(event3)
    event4 = Event("Event4", datetime.now(), \
                   "Afraid of dying in sky diving ", "Blank",\
                   8, 4, FEARAPP_STATUS_UPDATED, 0, user2.userid)
    db.session.add(event4)
    db.session.commit()
    import os
#    os.system('clear') : this statement causes a problem - server does not run
    qry = User.query.all()
    print ("\n\n---------------- ***** User db ***** ------------------")
